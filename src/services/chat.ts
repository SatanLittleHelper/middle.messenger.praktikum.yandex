...
export const addUserToChat = async (
  ...
  const users: Array<Record<string, any>> = await userAPI.searchUserByLogin(
    action,
...
export const deleteUserFromChat = async (
  ...
  if (currentChat.length > 0) {
    const userToDelete = currentChat[0].users.filter((
      user: Record<string, any>,
    ) => user.login === action.data.login);
    if (userToDelete.length === 0) {
      dispatch({
        Error: 'Can\\\'t find user in chat',
        messengerState: '',
      });
      return;
    }
...
export const getAllChats = async (
  ...
  const response: Array<Record<string, any>> = await chatAPI.getAllChats(
    chatsOffset,
  ) as unknown as Array<Record<string, any>>;
...
async function getChatUsers(chatId: string) {
  ...
  if (allChats?.length > 0) {
    ...
    chat[0].users = Object.values(
      response,
    )
      .filter((item) => item.id !== window.store.getState().user?.id);
  }
}
