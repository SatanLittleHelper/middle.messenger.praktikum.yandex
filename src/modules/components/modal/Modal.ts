  ...
  protected init() {
    ...
    this.props.events = {
      submit: (event: Event) => {
        ...
        const modalWithInputTextInput = this.children.modalWithInputText?.children.input.getContent()
          .querySelector('input');
        const uploadInput = this.children.uploadFile?.children.input.getContent()
          .querySelector('input');
        ...
        if (!modalWithInputTextInputError && !uploadFileInputError) {
          ...
          this.children.modalWithInputText?.children.input.setProps({
            error: modalWithInputTextInputError,
          });
